...
class NetworkDiscovery:
    def __init__(self, db_config):
        ...
        self.scan_ranges = [
            {
                'network': '10.200.0.0/16',  # Backbone
                'community': 'mpbn',
                'zone': 'Backbone',
                'vendor_hint': 'juniper'
            },
    ...
    def identify_type(self, sys_descr):
        ...
        if 'router' in descr_lower or 'mx' in descr_lower or 'ne' in descr_lower:
            ...
            return 'switch'
        elif 'microwave' in descr_lower or 'mini-link' in descr_lower:
            return 'microwave'
        return 'unknown'
    ...
    def scan_network_range(self, range_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            futures = {
                executor.submit(self.probe_device, str(ip), community): str(ip)
    ...
    def save_discovered_device(self, device):
        ...
        try:
            ...
            cur.execute("SELECT id FROM devices WHERE ip = %s", (device['ip'],))
            existing = cur.fetchone()
            
            if existing:
                cur.execute("""
                    UPDATE devices SET hostname=%s, vendor=%s, type=%s,
                                       last_seen=NOW(), sys_descr=%s
                    WHERE ip = %s
    ...
    def run_discovery(self):
        ...
        for range_config in self.scan_ranges:
            ...
            total += len(devices)
        logger.info(f"Discovery complete. Found {total} devices.")
        return total
